...
def create_trainer(preprocess_fn, network_factory, read_from_file, image_shape,
	...
	label_var = tf.compat.v1.disable_eager_execution()#tf.compat.v1.placeholder(tf.int64, (None,))
	...
	regularization_var = tf.reduce_sum(input_tensor=tf.compat.v1.losses.get_regularization_loss())
...
def eval_loop(preprocess_fn, network_factory, data_x, data_y, camera_indices,
	...
	log_dir: str
		Should be equivalent to the `log_dir` passed into `p` of the
	...
	num_probe = tf.shape(input=probe_x_var)[0]
...
def finalize(preprocess_fn, network_factory, checkpoint_path, image_shape, output_filename):
	...
	with tf.compat.v1.Session(graph=tf.Graph(),
					config=tf.compat.v1.ConfigProto(allow_soft_placement=True, log_device_placement=True)) as session:
		...
		saver.save(session, output_filename, global_step=None,save_format='h5')
...
def _create_encoder(preprocess_fn, network_factory, image_shape, batch_size=32,
	...
	if session is None:
		session = tf.compat.v1.Session(config=tf.compat.v1.ConfigProto(allow_soft_placement=True, log_device_placement=True))
...
def _create_softmax_loss(feature_var, logit_var, label_var):
	...
	accuracy_var = slim.metrics.accuracy(
		tf.cast(tf.argmax(input=logit_var, axis=1), tf.int64), label_var)
