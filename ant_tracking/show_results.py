...
import ant_tracking_app
from ant_tracking.iou_matching import iou
from application_util import visualization


DEFAULT_UPDATE_MS = 30
...
def run(sequence_dir, result_file, show_false_alarms=False, detection_file=None,
    ...
    seq_info = ant_tracking_app.gather_sequence_info(sequence_dir, detection_file)
    ...
    def frame_callback(vis, frame_idx):
        ...
        if seq_info["detections"] is not None:
            detections = ant_tracking_app.create_detections(
        ...
        if frame_idx > 2:
            vis.draw_centors(results, frame_idx)
